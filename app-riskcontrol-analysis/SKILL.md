---
name: app-riskcontrol-analysis
description: 用于在合法授权前提下分析移动 App / Native SO 的风控、反爬、设备指纹与加密参数还原工作流。涵盖 ARM64 BR 花指令去除、字符串解密器识别、JSON 节点 ABI 还原、设备指纹 xN 字段映射、四层加密栈（5 行明文 → 哈希 → 修改版 AES → RC4/HMAC 外层）、X25519 ECDH 密钥协商、修改版 MD5/HMAC、修改版 AES（Rcon/MixColumns/ShiftPerm/SubWord 改造）、Frida/Root/模拟器/Hook 框架/TEE Key Attestation 检测、code-patch-check 自校验、上传 schema (a/c/d/e/g/k/p/s/u/v) 还原与回放验证。当任务涉及 风控参数、风控分析、x-mini, x-mini-mua, x-mini-s1, x-mini-sig, shield, libtiny, libtiny.so, 设备指纹, device fingerprint, 加密参数还原, sig 参数, mua 参数, App 反爬, native SO 还原, BR 花指令, ARM64 obfuscation, frida hook, anti-frida, anti-debug, X25519, 修改版 AES, 修改版 MD5, 修改版 HMAC, RC4, ECDH, 5-line plaintext, /api/v1/profile/android, /api/v1/register/android, /api/v1/cfg/android, 小红书, Xiaohongshu, com.xingin.xhs，或一般性 "授权移动 App 风控/反作弊 SDK 分析" 时使用。
---

# App 风控参数分析（app-riskcontrol-analysis）
...
- 小红书 `libtiny.so` 全量逐字技术笔记：[references/xhs-libtiny-notes.md](references/xhs-libtiny-notes.md)。包含 8 类 BR 花指令的 ASM 示例与改写、字符串解密器伪码、SSO/JSON ABI、四个加密参数（`x-mini-mua / x-mini-s1 / x-mini-sig / shield`）的完整算法、`/api/v1/profile/android` 上传 schema、~200 个 `xN` 字段逐项注解、`code-patch-check` 自校验 9 段哈希原文、心跳两阶段位图、姊妹接口与 APM 旁路。
